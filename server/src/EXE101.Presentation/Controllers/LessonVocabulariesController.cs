...
[ApiController]
[Route("api/lesson_vocabularies")]
public sealed class LessonVocabulariesController(ILessonVocabularyService service) : ControllerBase
{
    [HttpGet]
    ...
    public async Task<IActionResult> GetById([FromRoute] long id, CancellationToken cancellationToken = default)
    {
        ...
        if (result is null)
        {
            return NotFound(new { message = $"Lesson vocabulary with id {id} was not found." });
        }
    ...
    [HttpPost]
    public async Task<IActionResult> Create([FromBody] CreateLessonVocabularyRequest request, CancellationToken cancellationToken = default)
    {
    ...
    [HttpPut("{id:long}")]
    public async Task<IActionResult> Update([FromRoute] long id, [FromBody] UpdateLessonVocabularyRequest request, CancellationToken cancellationToken = default)
    {
