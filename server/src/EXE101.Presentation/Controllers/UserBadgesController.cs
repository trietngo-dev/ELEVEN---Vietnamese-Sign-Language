...
[ApiController]
[Route("api/user_badges")]
public sealed class UserBadgesController(IUserBadgeService service) : ControllerBase
{
    [HttpGet]
    ...
    public async Task<IActionResult> GetById([FromRoute] long id, CancellationToken cancellationToken = default)
    {
        ...
        if (result is null)
        {
            return NotFound(new { message = $"User badge with id {id} was not found." });
        }
    ...
    [HttpPost]
    public async Task<IActionResult> Create([FromBody] CreateUserBadgeRequest request, CancellationToken cancellationToken = default)
    {
    ...
    [HttpPut("{id:long}")]
    public async Task<IActionResult> Update([FromRoute] long id, [FromBody] UpdateUserBadgeRequest request, CancellationToken cancellationToken = default)
    {
