...
[ApiController]
[Route("api/subscription_plans")]
public sealed class SubscriptionPlansController(ISubscriptionPlanService service) : ControllerBase
{
    [HttpGet]
    ...
    public async Task<IActionResult> GetById([FromRoute] long id, CancellationToken cancellationToken = default)
    {
        ...
        if (result is null)
        {
            return NotFound(new { message = $"Subscription plan with id {id} was not found." });
        }
    ...
    [HttpPut("{id:long}")]
    public async Task<IActionResult> Update([FromRoute] long id, [FromBody] UpdateSubscriptionPlanRequest request, CancellationToken cancellationToken = default)
    {
