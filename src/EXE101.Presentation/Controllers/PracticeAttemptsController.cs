...
[ApiController]
[Route("api/practice_attempts")]
public sealed class PracticeAttemptsController(IPracticeAttemptService service) : ControllerBase
{
    [HttpGet]
    ...
    public async Task<IActionResult> GetById([FromRoute] long id, CancellationToken cancellationToken = default)
    {
        ...
        if (result is null)
        {
            return NotFound(new { message = $"Practice attempt with id {id} was not found." });
        }
    ...
    [HttpPost]
    public async Task<IActionResult> Create([FromBody] CreatePracticeAttemptRequest request, CancellationToken cancellationToken = default)
    {
    ...
    [HttpPut("{id:long}")]
    public async Task<IActionResult> Update([FromRoute] long id, [FromBody] UpdatePracticeAttemptRequest request, CancellationToken cancellationToken = default)
    {
