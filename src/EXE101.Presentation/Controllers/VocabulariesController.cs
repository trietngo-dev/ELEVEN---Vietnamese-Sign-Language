...
public sealed class VocabulariesController(IVocabularyService service) : ControllerBase
{
    [HttpGet]
    ...
    public async Task<IActionResult> GetById([FromRoute] long id, CancellationToken cancellationToken = default)
    {
        ...
        if (result is null)
        {
            return NotFound(new { message = $"Vocabulary with id {id} was not found." });
        }
    ...
    [HttpPost]
    public async Task<IActionResult> Create([FromBody] CreateVocabularyRequest request, CancellationToken cancellationToken = default)
    {
    ...
    [HttpPut("{id:long}")]
    public async Task<IActionResult> Update([FromRoute] long id, [FromBody] UpdateVocabularyRequest request, CancellationToken cancellationToken = default)
    {
