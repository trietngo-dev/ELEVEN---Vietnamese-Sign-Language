...
[ApiController]
[Route("api/user_lesson_progress")]
public sealed class UserLessonProgressController(IUserLessonProgressService service) : ControllerBase
{
    [HttpGet]
    ...
    public async Task<IActionResult> GetById([FromRoute] long id, CancellationToken cancellationToken = default)
    {
        ...
        if (result is null)
        {
            return NotFound(new { message = $"User lesson progress with id {id} was not found." });
        }
    ...
    [HttpPost]
    public async Task<IActionResult> Create([FromBody] CreateUserLessonProgressRequest request, CancellationToken cancellationToken = default)
    {
