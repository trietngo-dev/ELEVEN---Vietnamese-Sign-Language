...
[ApiController]
[Route("api/payment_transactions")]
public sealed class PaymentTransactionsController(IPaymentTransactionService service) : ControllerBase
{
    [HttpGet]
    ...
    public async Task<IActionResult> GetById([FromRoute] long id, CancellationToken cancellationToken = default)
    {
        ...
        if (result is null)
        {
            return NotFound(new { message = $"Payment transaction with id {id} was not found." });
        }
    ...
    [HttpPost]
    public async Task<IActionResult> Create([FromBody] CreatePaymentTransactionRequest request, CancellationToken cancellationToken = default)
    {
    ...
    [HttpPut("{id:long}")]
    public async Task<IActionResult> Update([FromRoute] long id, [FromBody] UpdatePaymentTransactionRequest request, CancellationToken cancellationToken = default)
    {
