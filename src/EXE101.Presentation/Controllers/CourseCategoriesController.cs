...
public sealed class CourseCategoriesController(ICourseCategoryService service) : ControllerBase
{
    ...
    public async Task<IActionResult> GetById([FromRoute] long id, CancellationToken cancellationToken = default)
    {
        ...
        if (result is null)
        {
            return NotFound(new { message = $"Course category with id {id} was not found." });
        }
    ...
    [HttpPost]
    public async Task<IActionResult> Create([FromBody] CreateCourseCategoryRequest request, CancellationToken cancellationToken = default)
    {
    ...
    [HttpPut("{id:long}")]
    public async Task<IActionResult> Update([FromRoute] long id, [FromBody] UpdateCourseCategoryRequest request, CancellationToken cancellationToken = default)
    {
